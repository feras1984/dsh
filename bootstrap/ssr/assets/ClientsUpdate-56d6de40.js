...
const ClientsUpdate = ({ category, block }) => {
  ...
  useEffect(() => {
    blockService.getActiveBlocks(commonService.toSnakeCase(BlockCategories.INDUSTRIES)).then((response) => {
  ...
  const blockSchema = z.object({
    ...
    translations: z.record(z.string().length(2), z.object({
      ...
      // description: z.string().max(100, {message: " Description shouldn't exceed 500 characters!"})
    }))
  });
